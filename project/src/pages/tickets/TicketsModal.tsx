...
import { TicketDetailModal } from "./TicketDeatilModal";
...
export const TicketsModal: React.FC<TicketsProps> = ({
  vendedor,
  isOpenTickets,
  onClose,
}) => {
  ...
  const clientes = React.useMemo(
    () =>
      Array.isArray(clientesDb)
        ? clientesDb.filter((c) => c.vendedor_id === vendedor.id)
        : [],
    [clientesDb, vendedor.id]
  );
  ...
  return (
    ...
    >
      ...
      <TicketDetailModal
        isOpen={isOpenTicketDetail}
        onClose={() => setIsOpenTicketDetail(false)}
        ticket={ticketSeleccionado as Ticket}
        vendedor={vendedor}
      />
    </div>
  );
