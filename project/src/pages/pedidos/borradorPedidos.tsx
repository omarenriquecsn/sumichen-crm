...
import { useActualizarVendedor } from "../../hooks/useActualizarVendedor";
import ModalPorcentaje from "../../components/ui/ModalPorcentaje";
...
export const Pedidos: React.FC<PedidosProps> = ({
  ...
  const { mutate: actualizarVendedor } = useActualizarVendedor();
  ...
  // --- NUEVO: estado para modal de porcentaje de negociación ---
  const [modalPorcentajeVisible, setModalPorcentajeVisible] = useState(false);
  const [porcentajeNegociacion, setPorcentajeNegociacion] = useState<number>(0);
  const [pedidoPendienteAprobar, setPedidoPendienteAprobar] =
    useState<Partial<Pedido> | null>(null);
  // ------------------------------------------------------------
  ...
  // Al hacer clic en "Aprobar": guardamos pedido y abrimos modal para el porcentaje
  const handleAprobarPedido = (data: Partial<Pedido>) => {
    setPedidoPendienteAprobar(data);
    setPorcentajeNegociacion(0);
    setModalPorcentajeVisible(true);
  };

  // Se ejecuta cuando el modal confirma el porcentaje
  const handleConfirmarPorcentajeNegociacion = (porcentaje: number) => {
    if (!pedidoPendienteAprobar) {
      setModalPorcentajeVisible(false);
      toast.error("No hay pedido seleccionado para aprobar.");
      return;
    }

    const { productos_pedido, ...pedido } = pedidoPendienteAprobar;
    pedido.estado = "procesado";
    console.log("Pedido a aprobar:", productos_pedido);

    // Actualiza pedido en backend
    ...
    // Calcular monto de negociación según porcentaje y actualizar vendedor
    const montoTotal = Number(pedido.total) || 0;
    const montoNegociacion = Number(
      (montoTotal * (porcentaje / 100)).toFixed(2)
    );

    const elVendedor = Array.isArray(vendedoresDb)
      ? vendedoresDb.find((v) => v.id === pedido.vendedor_id)
      : null;
    if (elVendedor) {
      actualizarVendedor({
        id: elVendedor.id,
        datos: {
          ...elVendedor,
          monto_negociacion_mes:
            montoNegociacion + (elVendedor.monto_negociacion_mes || 0),
        },
      });
    }
    ...
    // Notificacion
    const clienteDelPedido = Array.isArray(clientes)
      ? clientes.find((c) => c.id === pedidoPendienteAprobar?.cliente_id)
      : null;
    crearNotificacion({
    ...
    setModalPorcentajeVisible(false);
    setPedidoPendienteAprobar(null);
    setPorcentajeNegociacion(porcentaje);
  ...
  return (
    ...
    >
      ...
      <ModalPorcentaje
        open={modalPorcentajeVisible}
        title="Porcentaje de Negociación"
        initialValue={porcentajeNegociacion}
        onClose={() => {
          setModalPorcentajeVisible(false);
          setPedidoPendienteAprobar(null);
        }}
        onConfirm={handleConfirmarPorcentajeNegociacion}
      />
    </Layout>
  );
};