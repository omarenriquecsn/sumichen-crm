...
23. **PWA instalable + Web Push + Acceso biométrico (28/08)** ✅: ver "Feature — PWA instalable + Notificaciones push + Acceso biométrico". App instalable en Android/iOS/Windows con notificaciones push por dispositivo y login con huella/rostro (WebAuthn). ✅ **Los eventos de negocio que disparan push ya están cableados** (Punto 17): pedidos (nuevo/aprobado/cancelado), leads (asignado/reasignado/perdido/convertido/sin vendedor/SLA), mensajes de chat, tickets, inventario y recordatorios de actividades. Las preferencias por evento viven en `preferencias_notificaciones` y se respetan en `enviarPushAUsuario`/`enviarPushAAdmins`/`enviarPushATodos` (`backend/src/services/pushServices.ts`).
24. **Acciones Rápidas del cliente rediseñadas + "Llamar" desde PC → móvil (29/08)** ✅: ver Punto 24 en §8. La sección "Acciones Rápidas" del detalle de cliente (`ClienteDetalle.tsx` y `ClienteDetalleModal.tsx`) se extrajo a un componente compartido con rejilla 2×2 de tarjetas (Llamar, Email, WhatsApp, Agendar Reunión, Crear Pedido) y zona de administración separada por divisor. Se corrigió el bug visual: en móvil "Llamar"/"Enviar Email" iban envueltos en `<a>` con `<button>` anidado (HTML inválido) y no llenaban el ancho. El botón "Llamar" en PC abre un popover con **"Copiar número"** y **"Enviar a mi móvil"** (Web Push al teléfono del usuario; al tocar la notificación la app abre `?accion=llamar` con un botón "Llamar ahora" — gesto del usuario requerido por iOS — que lanza `tel:`). Se descartó `navigator.share` porque el panel de Windows sale vacío. Nuevo tile **WhatsApp** (`wa.me/58...`).

## 8. Progreso de la actualización (bitácora)

Sesión de mantenimiento: **Punto 1 (Seguridad)** ✅, **Punto 2 (Migraciones)** ✅, **Punto 3 (Refactor de hooks)** ✅, **Punto 4 (Typos)** ✅, **Punto 5 (Rol consistente)** ✅, **Punto 6 (Rutas)** ✅, **Punto 7 (Lint/typecheck/test)** ✅, **Punto 8 (Google Calendar)** ✅, **Punto 9 (Imports)** ✅, **Punto 10 (SQL alineado)** ✅ y **Punto 11 (Fixes WhatsApp + UX, 22/08)** ✅. Plus: **Feature "Registrar Usuarios"** ✅, **Feature "Marketing / Leads (Fase 1)"** ⚠ (compila; fallos críticos corregidos — ver sección del feature), **Feature "Transporte y negociación en pedidos" (24/08)** ✅ (ver Punto 12), **Punto 13 (Próximas Actividades + modal detalle + persistencia de listas, 27/08)** ✅, **Punto 14 (Descargas DB: fix reuniones + columnas nuevas + descargas de marketing, 28/08)** ✅, **Punto 15 (PWA instalable + Web Push + Acceso biométrico, 28/08)** ✅ (ver "Feature — PWA instalable + Notificaciones push + Acceso biométrico"), **Punto 16 (Editar perfil propio, 28/08)** ✅, **Punto 17 (Notificaciones push por evento + recordatorios, 28/08)** ✅ y **Punto 18 (Fix overflow de selects en móvil, 29/08)** ✅, **Punto 19 (Fix catálogo PDF del asistente, 29/08)** ✅, **Punto 20 (Fix lógica y diseño de Analítica + seed de datos de prueba, 29/08)** ✅, **Punto 21 (Vista móvil de Reuniones: tarjetas + filtro que cabía en pantalla, 29/08)** ✅, **Punto 22 (Modernización del calendario de reuniones, 29/08)** ✅, **Punto 23 (Vista móvil de Tickets: tarjetas, 29/08)** ✅ y **Punto 24 (Acciones Rápidas del cliente + "Llamar" desde PC al móvil, 29/08)** ✅ y **Punto 26 (Personalizar el menú lateral desde Configuración, 01/09)** ✅ y **Punto 27 (Firma en Configuración + correo al cliente vía Resend con adjuntos, 01/09)** ✅, **Punto 28 (Disponibilidad de productos según inventario diario, 02/09)** ✅, **Punto 29 (Lista de precios PDF → `precio_base` + catálogo WhatsApp, 02/09)** ✅, **Punto 30 (Proyección de ventas por cliente, 08/09)** ✅ y **Punto 31 (Envío de correo con la cuenta de Gmail del vendedor vía OAuth2, 14/09)** ✅ y **Punto 32 (Fix pantalla en blanco por Service Worker + ErrorBoundary, 14/09)** ✅ y **Punto 33 (Fix de tipos en `utils/pedidos.ts` + `currentUser.rol`, 14/09)** ✅ y **Punto 34 (Limpieza total de errores de `tsc` + warning de lint, 14/09)** ✅.
...
- **Backend**:
  ...
  - **`utils/catalogoProductos.ts`** (nuevo): `generarCatalogoPDF()` descarga `inventario.xlsx` del bucket `inventario` (Supabase Storage), lo parsea con `exceljs` (detecta la columna "Descripción": busca el encabezado en la fila 1; si no existe, **fallback a la columna 2**, porque el inventario real NO tiene fila de encabezado — la fila 1 ya es un producto) y arma un PDF con `pdfkit` (encabezado **SUMICHEM · RIF J-406007986 · Contacto 0424-4368661**, título "Catálogo de Productos", lista numerada nombre+descripción). Los nombres se **deduplican** (el inventario tiene una fila por lote; el catálogo muestra cada producto una sola vez).
...
- 🔧 **Fix (29/08)** — "el PDF del catálogo no se creaba": el inventario real NO tiene fila de encabezado (la fila 1 ya es un producto) y la columna de código contiene fórmulas (`String(cell.value)` → `[object Object]`), así que `generarCatalogoPDF()` lanzaba `No se encontró la columna "Descripción"` y el asistente respondía "catálogo no disponible". Fix en `catalogoProductos.ts`: helper `textoCelda()` (resuelve celdas con fórmula/richText), fallback a columna 2 si no hay encabezado y **dedupe por nombre normalizado** (639 filas de lotes → 72 productos únicos). Verificado contra el inventario real: genera un PDF válido (build/lint/typecheck OK). Commit `90adda3`.
...
### Punto 16 — Editar perfil propio desde Configuración (28/08) ✅ (build/lint/typecheck OK backend y frontend)

> **Resumen**: el usuario autenticado puede actualizar su **propio perfil** (nombre, apellido, teléfono) desde Configuración → Perfil, sin que un admin tenga que hacerlo. El email NO es editable (vive en Supabase Auth).

- **Backend**:
  - `PUT /usuarios/perfil` (JWT) → `actualizarMiPerfilService`: busca al usuario por `supabase_id` (`req.user.id` = decoded.sub) y **solo** acepta `nombre`/`apellido`/`telefono`; cualquier otro campo del body se ignora por seguridad (rol/activo/supabase_id/email no se tocan).
  - ⚠ La ruta se declara **ANTES** de `/usuarios/:id` en `usuariosRoutes.ts` para que Express no interprete "perfil" como un `:id`.
  - Repositorio `updateUsuarioBySupabaseId` (busca por `supabase_id`, no por id de tabla).
- **Frontend**:
  - Nuevo hook `useActualizarPerfil.ts` → PUT `/usuarios/perfil` con Bearer token; invalida `userData`/`currentUser`/`vendedores`.
  - `Configuracion.tsx` tab Perfil: al "Guardar Cambios" se llama `useActualizarPerfil` + `refreshUserData()`.
- ⚠ Nota: `handleSeguridadChange` (cambiar contraseña) sigue usando `supabase.auth` y `perfilData.email` (pre-existente, sin cambios).

### Punto 17 — Notificaciones push por evento + recordatorios de actividades (28/08) ✅ (build/lint/typecheck OK backend y frontend)

> **Resumen**: los eventos de negocio ahora disparan **Web Push (PWA)** a los dispositivos donde el usuario instaló la app, y cada usuario decide qué notificaciones recibir desde Configuración → Notificaciones (toggles por evento, por cuenta). Se eliminan los toggles de email decorativos que no hacían nada. Se añade un worker que recuerda las actividades próximas.

#### Eventos cableados (13) — destinatarios y puntos
| Evento | Destinatario | Dónde |
|---|---|---|
| `pedido_nuevo` — nuevo pedido creado | Admins | `pedidosServices.createPedidosService` |
| `pedido_aprobado` — pedido confirmado (estado → `procesado`) | Vendedor dueño | `pedidosServices.updatePedidosService` |
| `pedido_cancelado` — pedido eliminado (se mantiene la eliminación) | Vendedor dueño + admins | `pedidosServices.deletePedidosService` |
| `lead_asignado` — "Dispones de 12 horas para atenderlo" | Vendedor asignado | `leadsServices.asignarLeadService` + `asistenteMenuServices.procesarRespuestaEstado` |
| `lead_reasignado` — reasignación manual o por SLA | Nuevo vendedor | `leadsServices.reasignarLeadService` + `procesarSLAVencidos` |
| `lead_perdido` | Admins | `leadsServices.perderLeadService` |
| `lead_nuevo_sin_asignar` — lead de WhatsApp sin vendedor | Admins | `whatsappWebhookServices` (rama `lead_creado`) |
| `sla_vencido_sin_vendedor` | Admins | `procesarSLAVencidos` (rama `sin_vendedor_zona`) |
| `lead_convertido` | Vendedor + admins | `leadsServices.convertirLeadService` |
| `ticket_nuevo` | Admins | `ticketsServices.createTicketsService` |
| `productos_actualizados` — inventario.xlsx subido a Supabase | Todos (admins + vendedores) | `productosControllers.subirInventario` |
| `actividad_proxima` — reunión/llamada/email | Vendedor dueño | `worker/recordatorios.ts` |
| `mensaje_nuevo` — mensaje del cliente en el chat | Vendedor de la conversación | `whatsappWebhookServices` + `conversacionesServices.recibirMensajeExternoService` |

#### Backend
- **Migración** `1787524209500-PreferenciasNotificacionesSchema.ts` (idempotente): tabla `preferencias_notificaciones` (PK `vendedor_id`+`evento`, FK CASCADE, `habilitado` default true; **si no hay fila el evento está habilitado**) + columnas `recordatorio_enviado`/`recordatorio_enviado_en` en `actividades`.
- **Entidad** `PreferenciaNotificacion` + **enum** `EventoNotificacionEnum` (los 13 keys estables, compartidos con el frontend) + **repositorio** (`obtenerPreferencias`, `obtenerPreferenciasDeVendedores`, `upsertPreferencias` batch con borrado de las que ya no están, `eventoHabilitado` default true).
- **Módulo** `GET/PUT /preferencias-notificaciones` (JWT) → `routes/preferenciasNotificacionesRoutes.ts` (montado en `indexRoutes`), controllers y services. El GET devuelve los 13 eventos con su estado (default true).
- **`pushServices.ts`**: `enviarPushAUsuario(vendedorDbId, payload, evento?)`, `enviarPushAAdmins(payload, evento?)` y nuevo **`enviarPushATodos(payload, evento?)`** (todos los vendedores activos). Si se pasa `evento`, se chequea la preferencia del usuario (en `enviarPushAAdmins`/`enviarPushATodos` se filtran los destinatarios que lo tienen deshabilitado en un solo query).
- **Worker `worker/recordatorios.ts`** (arranca en `index.ts` junto al SLA, graceful shutdown): cada `RECORDATORIO_INTERVALO_MIN` (default 5) busca actividades `!completado` tipo `llamada|email|reunion` con `fecha` en `[ahora, ahora+RECORDATORIO_MINUTOS]` y `recordatorio_enviado=false` → push `actividad_proxima` al vendedor + marca `recordatorio_enviado=true` (anti-duplicado). Las reuniones ya crean su `Actividad` (id_tipo_actividad), así que cubrir `actividades` es suficiente.
- **Env nuevo**: `RECORDATORIO_MINUTOS` (default 60) y `RECORDATORIO_INTERVALO_MIN` (default 5), documentados en `.env.example`.

#### Frontend
- **`constants/eventosNotificacion.ts`**: `CATEGORIAS_NOTIFICACIONES` (Pedidos/Leads/Chat/Catálogo/Agenda/Soporte) con los 13 eventos (etiqueta, descripción, icono lucide) y `EVENTOS_NOTIFICACIONES` (flat). Los valores `evento` deben coincidir con el enum del backend.
- **`lib/push.ts`**: `PreferenciaNotificacion`, `listarPreferencias()` y `guardarPreferencias(preferencias)` (GET/PUT con Bearer token).
- **`useNotificacionesPush.ts`**: nuevo hook `usePreferenciasNotificacion` (query `["push","preferencias"]`, `habilitado(evento)` con default true, `guardar(preferencias)` que refresca caché).
- **`Configuracion.tsx` tab Notificaciones**: se **eliminaron los 4 toggles de email decorativos** (`emailNuevosClientes`, `emailReuniones`, `emailTickets`, `resumenSemanal` — no hacían nada) y se reemplazaron por los toggles reales por evento agrupados por categoría. Cada cambio guarda automáticamente (toast de confirmación; con `prefsLocalesRef` para evitar carreras). Se mantiene intacta la sección "Notificaciones en este dispositivo" (activar/prueba/lista).

#### Cómo probar
- Suscribir un dispositivo (Configuración → Notificaciones → Activar). Desactivar un evento (ej. `pedido_nuevo`), dispararlo y verificar que NO llega; activarlo y verificar que llega (tabla `preferencias_notificaciones`).
- Eventos: crear/aprobar/cancelar pedido, asignar/reasignar/perder/convertir lead, subir `inventario.xlsx` (todos reciben), escribir mensaje de cliente en `/chat`, agendar una reunión/llamada a <1h (worker la avisa 1h antes).
- ⚠ La migración se aplica al arrancar el backend una vez (`npm run build` + reiniciar).

#### ⚠ Notas / deuda
- El "Cancelar pedido" sigue **eliminando** el pedido (decisión del usuario): el push `pedido_cancelado` se dispara al momento de borrar. No existe estado `cancelado` en `EstadoPedidoEnum`.
- El `url` de cada push apunta a rutas del HashRouter (`#/pedidos`, `#/chat/:id`, `#/leads`, `#/reuniones`, etc.); `sw.js` resuelve el hash contra el origen al hacer clic.
- Las preferencias son por **cuenta** (no por dispositivo). Se respetan en el backend en el momento del envío.
- El push de `pedido_nuevo` convive con el Pushover legacy (no se tocó).

### Punto 18 — Fix overflow de selects en móvil (29/08) ✅ (build/lint frontend OK)

> **Resumen**: en vista móvil, los `<select>` que vivían en filas flex horizontales conservaban su ancho intrínseco (los flex items no encogen sin `min-w-0`) y se salían de la pantalla. Se apilaron verticalmente a ancho completo en móvil (filas en desktop), sin tocar lógica ni estado.

- **`pages/marketing/MarketingDashboard.tsx`** (sección "Opciones de intención"): cada opción pasó de `flex items-center gap-2` a `flex flex-col sm:flex-row sm:items-center gap-2` con fondo sutil en móvil. El input de etiqueta → `w-full sm:flex-1 sm:min-w-0`; el `<select>` de tipo → `w-full sm:w-40 sm:shrink-0`; el botón de eliminar → `shrink-0 self-end sm:self-auto`.
- **`pages/clientes/Clientes.tsx`** y **`pages/clientes/ClientesModal.tsx`** (barra "Filtros"): el contenedor pasó de `flex items-center space-x-2` a `flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-0 sm:space-x-2`. El ícono `Filter` → `hidden sm:inline`; los selects "Todos los estados" y "Todos los sectores" → `w-full sm:w-auto sm:min-w-0`.
- ⚠ Nota: la persistencia de `filterSector`/`filterEstado` en `sessionStorage` (Punto 13) no se ve afectada (solo cambian clases Tailwind).

### Punto 19 — Fix catálogo PDF del asistente WhatsApp (29/08) ✅ (build/lint/typecheck OK backend)

> **Resumen**: el PDF del catálogo que envía el asistente cuando el cliente elige "Catálogo" **no se estaba creando**. La causa era `generarCatalogoPDF()` en `utils/catalogoProductos.ts`: asumía que la fila 1 del `inventario.xlsx` era el encabezado y buscaba una columna "Descripción"; el inventario real **no tiene fila de encabezado** (la fila 1 ya es un producto) y la columna de código contiene **fórmulas** (`String(cell.value)` → `[object Object]`), así que lanzaba `No se encontró la columna "Descripción"` y el asistente respondía "catálogo no disponible".

- **Causa raíz** (verificado 29/08): `inventario.xlsx` en el bucket `inventario` tiene 640 filas sin encabezado (col 1 = código con fórmulas, col 2 = descripción, cols 3-6 = globalca/wms/total/lote). La config `menu_bienvenida.opciones_intencion` ya tenía Catálogo y el enum `leads_tipo_web_enum` ya contenía `catalogo` → el flujo del chat sí llegaba a la rama de catálogo, pero `generarCatalogoPDF` fallaba antes de generar el buffer.
- **Fix** en `backend/src/utils/catalogoProductos.ts`:
  - Helper `textoCelda(cell)`: extrae el texto visible de celdas con fórmula (`v.result`), rich text (`v.text`/`v.richText`) u objetos; nunca devuelve `[object Object]`.
  - Detección de columna robusta: busca el encabezado normalizado que contenga `descripc` en la fila 1; si no aparece, **fallback a la columna 2** (formato real CODIGO | DESCRIPCION | GLOBALCA | WMS | TOTAL | LOTE, igual al `encabezado` de `Productos.tsx`).
  - **Dedupe por nombre normalizado**: el inventario tiene una fila por lote (639 filas); el catálogo muestra **72 productos únicos**.
- **Verificación**: `npm run build` + `typecheck` + `lint` OK; `generarCatalogoPDF()` contra el inventario real devuelve un PDF válido de ~4.9 KB (antes lanzaba el error). El envío a Meta (`sendWhatsAppDocument`) no se tocó y ya estaba OK (Node 22 con `FormData`/`Blob` globales verificado).
- ⚠ Para producción: desplegar el backend recompilado (`build/`). Para probar end-to-end se necesita una ventana de WhatsApp abierta (mensaje entrante → estado → "2" Catálogo).
- Commit: `90adda3`.

### Punto 20 — Fix lógica y diseño de Analítica + seed de datos de prueba (29/08) ✅ (build/lint/tsc OK frontend)

> **Resumen**: la página `/analitica` (vendedor) y el `AnaliticaModal` (admin en Vendedores) mostraban métricas inexactas: el "vs mes anterior" comparaba contra todo el histórico y contaba registros (no dinero), las barras de Ventas usaban escalas duras (`/50000`) o dividían entre 0 (`NaN`), el pipeline dividía un promedio por el total, las actividades usaban un hack `50/meta`, varios `cambio` eran strings literales que nunca renderizaban, y metas/actividades/recompras mezclaban unidades o meses. Se corrigió la lógica en las utils, se rediseñó el render (barras reales, estados vacíos, skeleton) y se creó un **seed SQL** de datos de prueba para visualizar la Analítica completa.

#### Correcciones de lógica (utils)

- **`utils/ventas.ts`**:
  - `calculoIncremento` (legacy) se **mantiene** exportado por compatibilidad con los dashboards (`DashboardAdmin`/`DashboardVendedor`/`DashboarVendedorModal`) pero deja de usarse en Analítica.
  - **Nuevos**: `incrementoEntreValores(actual, anterior)` (devuelve `null` sin base de comparación → se muestra "—"), `incrementoMensual(lista, getValor?)` (compara mes actual vs anterior con cruce de año; con `getValor` suma dinero, sin él cuenta registros), `tasaConversionMes(oportunidades, mes, anio)`, `incrementoConversionMensual(oportunidades)` (delta en **puntos porcentuales**).
  - `ventasPorMes`: filtra por **año actual** (antes contaba pedidos del mismo mes de años pasados).
  - `atras5meses`: reescrito con aritmética de índices → **siempre 5 meses** (Ene→Dic actual); rellena con 0 los meses del año anterior que faltan.
  - `recompras(pedidos, mes?, anio?)` reescrito: cuenta **clientes distintos con ≥2 pedidos procesados** en el periodo (antes contaba "órdenes extra": cliente con 3 pedidos sumaba 2). Nuevos helpers: `clientesRecurrentesMes` y `compradoresUnicosMes`.
- **`utils/clientes.ts`**: `clientesActualizadosMes` valida el **año** de `fecha_estado` (antes podía contar conversiones de años pasados); nuevo `clientesProspectosMesAnio(clientes, mes, anio)` para comparar prospectos mes a mes con cruce de año.
- **`utils/actividades.ts`** `actividadesPoCategoria`: `cantidad` = actividades **completadas del mes** de ese tipo (antes contaba creadas); `porcentaje` = `cantidad / meta * 100` (antes el hack `50/meta`). Retorna además `meta`; `porcentaje: null` si no hay meta (el render usa fallback relativo).
- **`utils/oportunidades.ts`** `clientePorEtapaAnalitica`: retorna `porcentaje` = **share** (`cantidad/total`) + `probabilidadPromedio` + `total` (antes `porcentaje` era el promedio de probabilidad y la barra lo dividía por el total → nunca se llenaba).
- **`hooks/useVentas.ts`**: `cifraVentasMes` filtra por año actual.

#### Render corregido (Analitica.tsx y AnaliticaModal.tsx — misma lógica)

- **Tarjetas (4)**: `cambio` ahora es número real (o `null` → "— vs mes anterior"); "Nuevos Prospectos" usa delta de **prospectos creados** (antes clientes activos, desalineado); "Tasa de Cierres" muestra `%` y delta en **puntos**; "Clientes Recurrentes" usa `recompras` mes a mes. Se eliminaron los strings literales tipo `"Number(incrementoClientes)"`.
- **Ventas Mensuales**: escala = **máximo de los 5 meses visibles** (nunca >100% ni NaN); la meta del mes se muestra como texto de referencia; etiquetas con `formatCurrencyCompacto`.
- **Pipeline por Etapas**: barra = share real (`cantidad/total`), texto secundario "prob. prom. X%".
- **Actividades por Tipo**: barra = `cantidad/meta*100` (o share relativo si no hay meta); muestra "meta: N".
- **Progreso de Metas**: `totalMetas()` uniforme (`llamadas+emails+reuniones+tareas`, antes el modal sumaba `ventas+clientes`); "Meta de Actividades" cuenta **completadas del mes** (antes de todos los meses → barra siempre llena); "Meta de Recompras" reemplaza el 20% inventado (`clientes.length*0.2`) por el ratio **recompras/compradores únicos**; si no hay metas → aviso "No hay metas configuradas para este mes".
- **Resumen de Rendimiento**: crecimiento real (o "—"), nuevos clientes y actividades completadas **del mes**.
- **UX**: skeleton de carga mientras cargan las queries y **estados vacíos** por sección ("Sin ventas...", "Sin oportunidades...", "Sin actividades completadas este mes", "No hay metas configuradas...").
- ⚠ La moneda NO se toca: el `total` del pedido siempre va en USD aunque el pago se registre en Bs (confirmado por el usuario).

#### Seed de datos de prueba (para visualizar la Analítica)

- **Nuevo `backend/scripts/seed_analitica.sql`** (idempotente, ejecutable con `psql` contra la DB dev `crm_local`): borra y recrea SOLO datos marcados (`empresa LIKE 'TEST-%'`, `notas='SEED-2026'`, `titulo LIKE 'TEST-%'`).
- Aplica a **Norangel Guedez** (`89a80c45-cd93-458a-8b9a-6646352f9cf7`), mes de prueba Agosto 2026: 1 meta (ventas 25k/clientes 20/llamadas 40/emails 25/reuniones 12/tareas 15), 28 clientes (12 prospectos repartidos Abr–Ago, 6 convertidos en Ago, 10 activos históricos), 30 pedidos procesados (Abr 3/May 4/Jun 5/Jul 6/Ago 12, ventas $4,630→$9,050→$12,100→$15,000→$25,000, 4 clientes recurrentes en Ago), 30 líneas de `productos_pedido` (productos reales), 28 actividades (22 completadas Ago) y 10 oportunidades (5 en Ago, 5 en Jul).
- **Cómo usar**: `& "C:\Program Files\PostgreSQL\18\bin\psql.exe" "postgres://postgres:postgres@localhost:5432/crm_local" -v ON_ERROR_STOP=1 -f backend/scripts/seed_analitica.sql`. Luego entrar a `/analitica` como Norangel. Re-ejecutar el mismo script borra y recrea (limpia).

### Punto 21 — Vista móvil de Reuniones: tarjetas + filtro que cabía en pantalla (29/08) ✅ (build/lint OK frontend)

> **Resumen**: en la página de reuniones (`/reuniones`) y en la variante modal (`ReunionesModal`, menú móvil del vendedor), el buscador + el `<select>` "Todas las reuniones" se salían de la pantalla en móvil (fila flex horizontal con ancho fijo `w-64`) y la lista era una `<table>` ancha con scroll horizontal. Se aplicó el mismo patrón que Clientes/Zonas/Leads: **tarjetas en móvil/tablet (`lg:hidden`) + tabla en desktop (`hidden lg:block`)**, y la barra de herramientas apila verticalmente en móvil.

- **Nuevo `project/src/components/ui/ReunionTarjeta.tsx`**: tarjeta móvil reutilizable (patrón `ClienteTarjeta`) con props `reunion`, `clienteEmpresa?`, `clienteTelefono?`, `onVer?`, `onEditar?`, `onCancelar?`. Muestra: icono de tipo (`Video`/`Phone`/`Users`/`Calendar`) en cuadro azul, título (`truncate`), descripción (`line-clamp-2`), badge de estado (`getEstadoColor`), filas con cliente (empresa + teléfono), fecha (`dddd D MMMM YYYY`), hora (`HH:mm a - HH:mm a`) y ubicación (`truncate`). Footer con acciones: "Ver" (azul, opcional), "Editar" (verde) y "Cancelar" (rojo, solo si `estado !== "cancelada"`, `ml-auto`).
- **`Reuniones.tsx`**:
  - Barra de herramientas: el bloque buscador+filtro pasa a `flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto`; input a `w-full sm:w-64`; `<select>` a `w-full sm:w-auto`; icono `Filter` oculto en móvil (`hidden sm:inline`). La fila derecha (Lista/Calendario + "Nueva Reunión") agrega `flex-wrap w-full sm:w-auto justify-between sm:justify-start`.
  - Lista: dentro de `vistaActual === "lista"`, tarjetas en `grid grid-cols-1 gap-3 lg:hidden` + tabla envuelta en `hidden lg:block`. Estado vacío "No se encontraron reuniones." cuando no hay resultados.
- **`ReunionesModal.tsx`**: mismos fixes de barra y lista, con el botón "Ver" en las tarjetas que abre `ReunionesDetailModal` (replicando el comportamiento de la tabla actual).
- **Verificación**: `npm run lint` 0 errores y `npm run build` OK (solo warnings pre-existentes).
- ⚠ Nota: dentro de la tabla desktop la indentación quedó a nivel de la versión anterior (solo cosmético; no afecta compilación ni lint, reglas relajadas para legacy).

### Punto 22 — Modernización del calendario de reuniones (29/08) ✅ (build/lint OK frontend)

> **Resumen**: el calendario de reuniones (`Calendario.tsx`) se rediseñó por completo (aspecto moderno, toolbar personalizado, chips con color por estado e icono por tipo, leyenda, filtros) y se agregó funcionalidad: **crear desde un slot** (clic en día/hora → flujo de "Nueva Reunión" con fecha precargada), **drag & drop / redimensionar** para reprogramar en PC (addon `withDragAndDrop`, ya incluido en `react-big-calendar`) y **reprogramar por toques en móvil** (el drag HTML5 no funciona en táctil → botón "Editar / Reprogramar" en el modal de detalle que abre el formulario de edición con fecha/hora precargadas).

#### Aspecto moderno
- **Nuevo `project/src/pages/reuniones/calendario.css`**: overrides de `.rbc-*` — bordes redondeados, sombras suaves, cabeceras con fondo `gray-50`, celda "hoy" con número en círculo azul, fines de semana suaves (`#fafafa`), overlay "+N más" estilizado, scrollbar sutil.
- **Toolbar personalizado** (`components.toolbar`, componente `ToolbarCalendario`): `ChevronLeft/ChevronRight` + botón "Hoy" + título del mes en español + selector de vista segmentado (Mes/Semana/Día/Agenda) con el mismo patrón que Lista/Calendario.
- **Eventos como chips** (`components.event`, componente `EventoCalendario`): color por estado vía `eventPropGetter` (clases CSS `chip-programada` azul, `chip-completada` verde, `chip-cancelada` roja con `line-through`), icono lucide por tipo (presencial/virtual/telefónica), hora + título truncado. Las canceladas se ven tachadas.
- **Leyenda** de colores (Programada/Completada/Cancelada) + hint contextual por dispositivo.
- **Filtros internos** por estado y tipo (selects sobre los eventos cargados).
- Se quitó de `Reuniones.tsx`/`ReunionesModal.tsx` la caja gris con el icono gigante + título "Calendario" redundante; el calendario ahora ocupa todo el alto de la tarjeta.
- `moment.locale("es")` se establece a nivel de módulo (solo se usa moment en este componente).

#### Funcionalidad
- **Crear desde slot**: `selectable="ignoreEvents"` + `onSelectSlot` → prop `onSlotSelect(start)`. En los padres (`Reuniones.tsx`, `ReunionesModal.tsx`) se guarda `slotInicial` y se pasa `initialData={{ fecha_inicio, fecha_fin }}` al `CrearReunion` del flujo de creación (fecha + hora de inicio/fin precargadas, fin = inicio + 1h).
- **Drag & drop / resize en PC**: `withDragAndDrop(Calendar)` (tipos ya vienen en `@types/react-big-calendar`, no hace falta `.d.ts`). `onEventDrop`/`onEventResize` → `useActualizarReunion` con nuevas `fecha_inicio`/`fecha_fin` + toast ("Reunión reprogramada"). `draggableAccessor`/`resizableAccessor` excluyen las canceladas.
- **Móvil (drag HTML5 no soportado en táctil)**: helper `esDispositivoTactil()` (`matchMedia("(pointer: coarse)")`/`ontouchstart`/`maxTouchPoints`) → en táctil se desactiva el DnD (`draggableAccessor={() => false}`, `resizable={false}`) y el hint cambia a "Toca un evento para ver o reprogramar." `ReunionesDetailModal` recibe nueva prop opcional `onEditar` que muestra el botón **"Editar / Reprogramar"** (icono `Pencil`, oculto si `estado === "cancelada"` y si no se pasa la prop). Los padres la cablean con `onEditarReunion` → abre su modal `CrearReunion` con la fecha/hora de la reunión precargadas (misma mutación que el drag).
- **Fix id NaN**: los eventos usan `reunion.id` (uuid real) como id/key (antes `Number(uuid)` = `NaN` → keys duplicadas de React).
- **Otros**: semana inicia lunes + formato español (`culture="es"` + `formats` con moment), horas visibles 07:00–20:00 (`min`/`max`) en vistas día/semana, `longPressThreshold={30}`, estados de carga (spinner) y vacío ("No hay reuniones que mostrar con los filtros actuales").
- **Unificación**: `Calendario.tsx` ahora es el único componente (prop opcional `vendedorId` = patrón `useApi`); `CalendarioModal.tsx` **se eliminó** y `ReunionesModal.tsx` usa `<Calendario vendedorId={vendedor.id} />`.

#### Verificación
- `npm run lint` 0 errores y `npm run build` OK (solo warnings pre-existentes de chunk size / import dinámico).

#### ⚠ Notas / deuda
- El drag & drop sigue siendo HTML5 nativo: **solo funciona con mouse/PC** (limitación del addon). En táctil la alternativa es tocar → "Editar / Reprogramar". Si algún día se quiere drag táctil real, habría que reimplementarlo con `@dnd-kit` (instalado) o un backend táctil propio (mucho trabajo/riesgo; ver decisión en sesión).
- `ReunionesDetailModal` con `onEditar` es opcional y backward-compatible: en el listado de `ReunionesModal` (donde no se pasa) no aparece el botón.

### Punto 23 — Vista móvil de Tickets: tarjetas (29/08) ✅ (build/lint OK frontend)

> **Resumen**: en `/tickets` y en el `TicketsModal` (menú móvil del vendedor), la lista era una `<table>` ancha con scroll horizontal (`overflow-x-auto`) que se salía de la pantalla en móvil. Se aplicó el mismo patrón que Clientes/Zonas/Leads/Reuniones: **tarjetas en móvil/tablet (`lg:hidden`) + tabla en desktop (`hidden lg:block`)** y el buscador pasa a ancho completo en móvil.

- **Nuevo `project/src/components/ui/TicketTarjeta.tsx`**: tarjeta móvil reutilizable (patrón `ReunionTarjeta`) con props `ticket`, `clienteEmpresa?`, `clienteEmail?`, `clienteTelefono?`, `onVer?`, `onResolver?`. Muestra: icono de estado (`AlertCircle`/`Clock`/`CheckCircle`/`XCircle` coloreado) en cuadro azul, título (`truncate`), descripción (`line-clamp-2`), badge de estado (`getEstadoColor` + `replace("_"," ")`), filas con cliente (empresa + email/teléfono), badges de prioridad (`getPrioridadColor`) y categoría (`getCategoriaColor`), y fecha actualizada (`dayjs` "DD/MM/YYYY"). Footer con acciones: "Ver" (azul, opcional) y "Resolver" (verde, solo si `estado !== resuelto/cerrado`, `ml-auto`). Reutiliza `utilsTikets()` para los colores.
- **`Tickets.tsx`**: la lista ahora usa **tarjetas en `grid grid-cols-1 gap-3 lg:hidden`** + la tabla envuelta en `hidden lg:block`. En las tarjetas `onVer` → `navigate(`/tickets/${ticket.id}`)` (mismo comportamiento que la tabla) y `onResolver` → `prepararCancelacion(ticket.id)`. El input de búsqueda pasa de `w-64` fijo a `w-full sm:w-64` (los selects ya apilaban bien del Punto 18).
- **`TicketsModal.tsx`**: mismo cambio de lista, con `onVer` → `setTicketSeleccionado(ticket)` + `setIsOpenTicketDetail(true)` (abre el `TicketDetailModal` anidado, replicando el comportamiento actual de la tabla) y `onResolver` → `prepararCancelacion(ticket.id)`. Buscador a `w-full sm:w-64`.
- **Verificación**: `npm run lint` 0 errores y `npm run build` OK (solo warnings pre-existentes de chunk size / import dinámico).
- ⚠ Nota: sin cambios de lógica ni estado; solo clases Tailwind + el componente nuevo de tarjeta (mismo patrón del Punto 21).

### Punto 24 — Acciones Rápidas del cliente rediseñadas + "Llamar" desde PC al móvil (29/08) ✅ (build/lint/typecheck OK backend y frontend)

> **Resumen**: la sección "Acciones Rápidas" del detalle de cliente (`ClienteDetalle.tsx` y `ClienteDetalleModal.tsx`, que estaban duplicadas) se extrajo a un **componente compartido** `components/ui/AccionesRapidasCliente.tsx` con **rejilla 2×2 de tarjetas** (icono en círculo de color + etiqueta) y zona de **Administración** separada por divisor. Se corrigió el **bug visual**: en móvil "Llamar" y "Enviar Email" iban envueltos en `<a>` (inline) con `<button>` anidado (HTML inválido) y no llenaban el ancho. Además, el botón "Llamar" ahora permite **disparar la llamada desde el PC en el móvil** mediante Web Push (PWA).

#### Rediseño de la sección (visual)
- **Nuevo `components/ui/AccionesRapidasCliente.tsx`**: props `cliente`, `isMobile`, `esAdmin`, `onLlamar`, `onEmailMovil`, `onEmailDesktop`, `onAgendarReunion`, `onCrearPedido`, `onAsignarVendedor?`, `onEliminarDefinitivamente?`. Rejilla `grid grid-cols-2 gap-3` de tarjetas (funciona en móvil a ancho completo y en el sidebar de PC ~150px por tarjeta):
  - **Llamar** (azul, `Phone`): solo si hay teléfono. Móvil → `<a href="tel:">` + registra actividad "llamada". PC → botón que abre un **popover** (cierre por clic fuera/Escape) con "Copiar número" y "Enviar a mi móvil".
  - **Enviar Email** (verde, `Mail`): móvil → `mailto:` + registra actividad; PC → `abrirGmail()`.
  - **WhatsApp** (esmeralda, `MessageCircle`, **nuevo**): `<a href="wa.me/58...">` con saludo precargado; normaliza el teléfono a formato país (`0` → `58`; sin `58` → lo antepone).
  - **Agendar Reunión** (morado, `Calendar`) y **Crear Pedido** (naranja, `FileText`): abren sus modales.
  - Zona **Administración** (solo `esAdmin`, separada por `border-t`): "Asignar Vendedor" (outline azul) y "Eliminar Definitivamente" (outline rojo).
- Las tarjetas enlace son el propio `<a>` (flex), sin `<button>` anidado → ancho uniforme y HTML válido.
- `ClienteDetalle.tsx` y `ClienteDetalleModal.tsx`: el bloque duplicado se reemplazó por `<AccionesRapidasCliente .../>` (handlers intactos; solo se limpió el import huérfano `UserCheck`).

#### "Enviar a mi móvil" (llamada cross-device vía Web Push)
> Desde el PC, el vendedor puede disparar la llamada en su teléfono: envía un push a sus dispositivos (excluyendo el que origina), toca la notificación y la app abre "Llamar ahora" que lanza `tel:`.

- **Backend**:
  - `services/pushServices.ts`: `enviarLlamadaAlMovil(vendedorDbId, { telefono, nombre?, clienteId, endpointOrigen? })` — normaliza el teléfono a dígitos, lista las suscripciones del usuario y **excluye `endpointOrigen`** (la del PC que dispara); envía `{ titulo: "📞 Llamar a {nombre}", cuerpo: telefono, url: "./#/clientes/{clienteId}?accion=llamar&telefono=..." }` y devuelve `{ enviadas, total }`. No pasa por `preferencias_notificaciones` (acción manual, como el push de prueba).
  - `controllers/pushControllers.ts`: `enviarLlamadaController` (valida `telefono`/`clienteId`, usa `req.user.vendedor_db_id`).
  - `routes/pushRoutes.ts`: `POST /push/enviar-llamada` (JWT + asyncHandler).
- **Frontend**:
  - `lib/push.ts`: `enviarLlamadaMovilFront({ telefono, clienteId, nombre? })` — obtiene el endpoint de la suscripción local (`pushManager.getSubscription()`, puede ser `undefined` si el PC no está suscrito) y hace el POST.
  - `AccionesRapidasCliente.tsx`: opción "Enviar a mi móvil" (icono `Smartphone`, estado "Enviando..." mientras dura). Toast de éxito con nº de dispositivos o error claro si `enviadas === 0` ("No encontramos tu móvil con la app...").
  - **Nuevo `components/ui/LlamarDesdePush.tsx`** (montado dentro de `<Router>` en `App.tsx`): lee `useSearchParams`; si `accion === "llamar"` muestra un modal con "Llamar a {número}" y botón **"Llamar ahora"** que ejecuta `window.location.href = "tel:..."` y limpia el query (`setSearchParams({}, { replace: true })`). El botón es necesario porque **iOS bloquea `tel:` sin gesto directo del usuario** en la página.
- `sw.js` **sin cambios**: su `notificationclick` ya navega a la URL con el hash (`./#/clientes/:id?accion=llamar...`).
- Se **eliminó "Compartir a mi móvil"** (`navigator.share`): en Windows el panel de compartición del sistema sale **vacío** (no hay apps de share registradas).

#### Verificación
- `npm run lint` 0 errores y `npm run build` OK (frontend, solo warnings pre-existentes); `npm run typecheck` + `npm run build` + `npm run lint` OK (backend).

#### Cómo probar end-to-end
1. Teléfono: app instalada (PWA) + sesión iniciada + notificaciones activas (Configuración → Notificaciones → "Activar"). Verificar que aparece en `push_suscripciones`.
2. PC: detalle de cliente → "Llamar" → "Enviar a mi móvil" → toast de éxito.
3. Teléfono: tocar la notificación "📞 Llamar a ..." → la app abre con el modal "Llamar al cliente" → "Llamar ahora" → marcador.

#### ⚠ Notas / deuda
- Requiere que el mismo usuario esté logueado y suscrito en el móvil; si no hay dispositivo (o solo está el PC), `enviadas === 0` y el toast lo explica.
...
### Punto 26 — Personalizar el menú lateral (sidebar) desde Configuración (01/09) ✅ (build/lint/typecheck OK backend y frontend)

> **Resumen**: nueva pestaña **"Menú lateral"** en `/configuracion` donde **cada usuario** (admin o vendedor) decide qué secciones quiere ver en su propio sidebar. Lo que se desactiva desaparece del menú lateral de esa cuenta (la ruta sigue siendo accesible por URL, decisión del usuario). "Configuración" **nunca** se puede ocultar para que nadie quede atrapado sin acceso a la configuración. Default: todo visible (los usuarios existentes no cambian su menú).

#### Backend
- **Migración** `1787524210000-SidebarPreferenciasSchema.ts` (idempotente): `ALTER TABLE vendedores ADD COLUMN IF NOT EXISTS sidebar_oculto jsonb NOT NULL DEFAULT '[]'::jsonb`. Se aplica al arrancar el backend (`migrationsRun`).
- **Entidad** `Vendedores.ts`: `@Column({ type: 'jsonb', nullable: false, default: () => "'[]'::jsonb" }) sidebar_oculto: string[]`.
- **`actualizarMiPerfilService`** (`usuariosServices.ts`): acepta `sidebar_oculto` (valida que sea array de strings, normaliza trims y deduplica; permite vacío). Se reutiliza `PUT /usuarios/perfil` (endpoint de auto-perfil, JWT). El `GET /usuarios/:id` ya devuelve la entidad completa → el campo llega solo, sin cambios de controlador.

#### Frontend
- **Nuevo `constants/menuSidebar.ts`**: `MenuLink`, `vendedorLinks`, `adminLinks`, `adminPrincipalLink` y `obtenerLinksMenu(rol, esOmar)` — única fuente de verdad del menú (se movió desde `Sidebar.tsx`, que ahora lo importa; comportamiento idéntico).
- **Tipos**: `UserData.sidebar_oculto?: string[]` (`context/types.ts`), `Vendedor.sidebar_oculto?: string[]` (`types/index.ts`), y mapeo en `useUserData.ts` (`sidebar_oculto: usuarioDB.sidebar_oculto ?? []`).
- **Nuevo `hooks/useActualizarSidebar.ts`**: mutation → `PUT /usuarios/perfil` con `{ sidebar_oculto }`; invalida `["userData"]`/`["currentUser"]`.
- **`Sidebar.tsx`**: filtra los links con `userData.sidebar_oculto` (Set de rutas); `/configuracion` siempre visible.
- **`Configuracion.tsx`**: pestaña **"Menú lateral"** (icono `LayoutGrid`) con toggle switch por ítem (mismo estilo que Notificaciones), auto-guardado al tocar cada toggle (patrón ref anti-carreras como `handleToggleEvento`), toast de confirmación, "siempre visible" en Configuración y spinner "Guardando...". Disponible para todos los usuarios (cada uno personaliza el suyo).

#### Cómo probar
- Loguear como vendedor y como admin → `/configuracion` → "Menú lateral" → desactivar/activar secciones → el sidebar se actualiza al instante (invalidación de `userData`) y persiste al recargar (columna `vendedores.sidebar_oculto`). Verificar en DB con `SELECT sidebar_oculto FROM vendedores;`.
- Intentar ocultar "Configuración" → el toggle está deshabilitado.

#### ⚠ Notas / deuda
- La sección **solo oculta el menú lateral**; las rutas no se bloquean (decisión del usuario). Si algún día se quiere bloquear la URL, habría que añadir lógica en el router.
...
- **Punto 15 / PWA + Web Push + Biometría** ✅ (28/08): el CRM es ahora una **PWA instalable** (Android/iOS/Windows) con botón "Instalar app" en el Sidebar (visible solo si no está instalada), **notificaciones push por dispositivo** (Web Push con VAPID; suscripciones en `push_suscripciones`; UI en Configuración → Notificaciones con prueba de punta a punta) y **acceso biométrico** (WebAuthn/passkey: registro en Configuración → Seguridad y "Entrar con huella" en el Login). Migraciones `1787524209200` y `1787524209300` **aplicadas en dev**. Ver Punto 15 en §8.
- **Punto 16 / Editar perfil propio** ✅ (28/08): `PUT /usuarios/perfil` (JWT, declarado antes de `/usuarios/:id`) permite al usuario autenticado actualizar nombre/apellido/teléfono desde Configuración → Perfil (email no editable; campos no permitidos se ignoran). Ver Punto 16 en §8.
- **Punto 17 / Notificaciones push por evento** ✅ (28/08): los **13 eventos de negocio ya disparan Web Push** a los dispositivos donde se instaló la app (pedidos nuevo/aprobado/cancelado, leads asignado/reasignado/perdido/convertido/sin vendedor/SLA, mensajes de chat, tickets, inventario actualizado y recordatorios de actividad próxima). Cada usuario elige qué recibir desde Configuración → Notificaciones (toggles por evento, guardados en `preferencias_notificaciones`; default habilitado). Migración `1787524209500` se aplica al arrancar el backend. Env nuevos `RECORDATORIO_MINUTOS` (default 60) y `RECORDATORIO_INTERVALO_MIN` (default 5). Ver Punto 17 en §8.
- **Punto 18 / Fix overflow de selects en móvil** ✅ (29/08): selects que se salían de la pantalla en móvil. En `MarketingDashboard.tsx` (opciones de intención) y `Clientes.tsx`/`ClientesModal.tsx` (barra de filtros) las filas flex ahora apilan verticalmente en móvil a ancho completo (`flex-col sm:flex-row`) con los selects a `w-full sm:w-auto`, mientras que en desktop mantienen el aspecto en fila. Sin cambios de lógica ni estado. Ver Punto 18 en §8.
- **Punto 19 / Fix catálogo PDF del asistente** ✅ (29/08): el PDF del catálogo no se generaba porque `generarCatalogoPDF()` asumía encabezado en la fila 1; el `inventario.xlsx` real **no tiene fila de encabezado** (la fila 1 ya es un producto) y la columna de código son fórmulas (`[object Object]`). Fix en `utils/catalogoProductos.ts`: helper `textoCelda()`, **fallback a columna 2** (DESCRIPCION) y **dedupe por nombre** (639 filas de lotes → 72 productos únicos). `generarCatalogoPDF()` verificado contra el inventario real (PDF válido ~4.9 KB; antes lanzaba el error y el asistente respondía "catálogo no disponible"). Ver Punto 19 en §8.
- **Punto 20 / Fix de Analítica + seed de datos** ✅ (29/08): la página `/analitica` y el `AnaliticaModal` ahora muestran la realidad: incrementos **mes vs mes** con dinero real (`incrementoMensual`), barras de Ventas escaladas al máximo visible (sin `/50000` ni `NaN`), Pipeline con **share real** por etapa, Actividades con `cantidad/meta` real (completadas del mes), tarjetas con `cambio` numérico real, `totalMetas()` uniforme, "Meta de Actividades" del mes actual y "Meta de Recompras" = recompras/compradores únicos (se quitó el 20% inventado). Diseño con skeleton y estados vacíos. Nuevo **seed** `backend/scripts/seed_analitica.sql` (idempotente, solo datos `TEST-`) que carga metas/clientes/pedidos/actividades/oportunidades para Norangel Guedez en Ago 2026. Ver Punto 20 en §8.
- **Punto 21 / Vista móvil de Reuniones** ✅ (29/08): en `/reuniones` y `ReunionesModal` el buscador + el filtro "Todas las reuniones" se salían de la pantalla en móvil y la lista era una tabla ancha con scroll horizontal. Fix: **nuevo `components/ui/ReunionTarjeta.tsx`** (patrón `ClienteTarjeta`, con acciones Ver/Editar/Cancelar) y la lista ahora usa **tarjetas en móvil/tablet (`lg:hidden`) + tabla en desktop (`hidden lg:block`)**, mismo patrón que Clientes/Zonas/Leads. La barra de herramientas apila verticalmente en móvil (input y select a `w-full sm:w-auto`). Sin cambios de lógica ni estado. Ver Punto 21 en §8.
...
- **Punto 26 / Personalizar el menú lateral desde Configuración** ✅ (01/09): nueva pestaña **"Menú lateral"** en `/configuracion` donde **cada usuario** (admin o vendedor) activa/desactiva las secciones de su propio sidebar (lo oculto no se muestra en el menú; la ruta sigue accesible por URL; "Configuración" siempre visible). Default todo visible → los usuarios existentes no cambian. Se guarda en `vendedores.sidebar_oculto` (jsonb) vía `PUT /usuarios/perfil` (validado en `actualizarMiPerfilService`). Menú centralizado en `constants/menuSidebar.ts` (`obtenerLinksMenu`), `Sidebar.tsx` filtra con `userData.sidebar_oculto`, y nuevo `hooks/useActualizarSidebar.ts`. Migración `1787524210000-SidebarPreferenciasSchema.ts` (idempotente) se aplica al arrancar el backend. Ver Punto 26 en §8.
- **Punto 31 / Envío de correo con la cuenta de Gmail del vendedor (OAuth2)** ✅ (14/09): cada vendedor conecta su propia cuenta de Gmail desde **Configuración → Perfil** ("Conectar con Google") y sus correos salen desde esa cuenta (quedan en "Enviados") usando la **API de Gmail**; si no hay cuenta conectada se usa **Resend** como respaldo. Migración `1787524214000-GoogleOAuthSchema.ts` (4 columnas en `vendedores`; los tokens con `select:false` para no exponerse por API) **aplicada en dev**. Nuevos: `utils/googleOAuth.ts`, `services/googleAuthServices.ts`, `services/gmailServices.ts`, módulo `/auth/google` (url/status/callback público/delete) y `hooks/useGoogleAuth.ts`. Deps `googleapis` + `nodemailer`. Env: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URL`, `GOOGLE_STATE_SECRET`, `FRONTEND_URL`. ⚠ Para probar: registrar el redirect en Google Cloud y completar las credenciales en `backend/.env`. Ver Punto 31 en §8.
- **Punto 32 / Fix pantalla en blanco por Service Worker + ErrorBoundary** ✅ (14/09): el handler `fetch` del SW devolvía `index.html` para cualquier recurso que fallara (incluidos `.js`/`.css`) → tras un deploy el bundle viejo fallaba y el navegador intentaba ejecutar HTML como JS → pantalla en blanco persistente. Fix: **se eliminó el handler `fetch`/caché del `sw.js`** (el CRM es online-first; solo quedan `push`/`notificationclick` y la limpieza de cachés en `activate`) y se añadió un **`ErrorBoundary`** en `main.tsx` con botón **"Reparar y recargar"** (desregistra SW + borra cachés). `dist/sw.js` verificado sin handler de `fetch`. Deploy: subir el `dist/` nuevo (auto-repara clientes rotos) y servir `sw.js` con `Cache-Control: no-cache`. Ver Punto 32 en §8.
- **Punto 33 / Fix de tipos en `utils/pedidos.ts` + `currentUser.rol`** ✅ (14/09): se corrigieron los 5 errores de `tsc` en `src/utils/pedidos.ts` (+1 en `types/index.ts`) sin cambios de runtime. `Pedido.transporte_detalle` ahora es `Transporte | Partial<Transporte>` (arregla el TS2430 de `PedidoData` y los TS2322 al pasar `pedidoData`), y `AuthContextType.currentUser` se tipó como `User & { rol?; supabase_id? }` (es el perfil de `vendedores` en runtime) + se ampliaron las props `vendedor` en los modales. Total de errores `tsc`: 48 → 19 (los 19 restantes son pre-existentes y ajenos). Ver Punto 33 en §8.
- **Punto 34 / Limpieza total de errores de `tsc` + warning de lint** ✅ (14/09): se eliminaron los 19 errores pre-existentes que quedaban (`Uint8Array<ArrayBuffer>` en `lib/biometric.ts`/`lib/push.ts`, `m.clientes` en `Analitica*`, formatter de recharts en `DashboardVendedor`, comparación muerta en `ActividadDetalleModal`, y `withDragAndDrop<CalendarioEvento>` + cast de `resource` en `Calendario.tsx`) y el único warning de lint (`ChatLista.tsx` con `useMemo`). Resultado: **`tsc` 0 errores, `lint` 0 problemas, `build` OK**. Ver Punto 34 en §8.
...
- **`utils/catalogoProductos.ts`** (reescrito): `generarCataÁrea de Inventariora lee `lista_precios.pdf` del disco y arma el PDF de cliente con pdfkit **multi-página** (cabecera SUMICHEM/RIF/contacto, encabezado de columnas por página, filas con salto automático) con **Código · Producto · Presentación**, dedupe de filas idénticas. Se eliminó la lectura de `inventario.xlsx` de Supabase Storage.
- `backend/.env.example`: `PRODUCTOS_UPLOAD_PATH` documentado.

#### Frontend
- **`ExcelProductos.tsx`**: 2 tarjetas separadas. Bloque 1 "Área de Edmary — Cargar Productos en almacén" (excel, igual). Bloque 2 "Área de Ventas — Cargar productos con precio base" (`accept=".pdf"`, campo `filePrecios` → `POST /productos/lista-precios`). Toast de éxito con `actualizados/sinCambio` del resumen y aviso si ningún código coincidió; códigos sin coincidencia a `console.warn`.
...
### Punto 31 — Envío de correo con la cuenta de Gmail del vendedor (OAuth2) (14/09) ✅ (build/lint/typecheck OK backend y frontend; migración aplicada en dev)

> **Resumen**: cada vendedor puede **conectar su propia cuenta de Gmail** (ej. `norangel.guedez.sumichemint@gmail.com`) desde **Configuración → Perfil**. Cuando la conecta, los correos que envía al cliente (modal "Enviar Email" del detalle de cliente) salen **desde esa cuenta** usando la **API de Gmail**, por lo que quedan guardados en su carpeta **"Enviados"**. Si el vendedor NO conectó una cuenta, se mantiene el envío por **Resend** (dominio verificado) como respaldo. Se usa la API de Gmail (no SMTP) precisamente para que el correo quede en "Enviados".

#### Backend
- **Dependencias**: `googleapis` (OAuth2 + Gmail API) y `nodemailer` (+ `@types/nodemailer`, solo para `MailComposer`, que arma el MIME con HTML, adjuntos y firma inline por CID).
- **Migración** `1787524214000-GoogleOAuthSchema.ts` (idempotente): agrega a `vendedores` las columnas `google_email` (varchar), `google_refresh_token` (text), `google_access_token` (text) y `google_token_expiry` (timestamptz). **Aplicada en dev**.
- **Entidad `Vendedores.ts`**: 4 campos nuevos. Los 3 tokens van con **`select: false`** para que NUNCA se serialicen en las respuestas de la API (`GET /usuarios` y `GET /usuarios/:id` devuelven la entidad completa); solo se leen explícitamente con QueryBuilder (`getUsuarioConGoogleByIdDb` / `...BySupabaseId` en `usuariosRepository.ts`).
- **`utils/googleOAuth.ts`**: `SCOPES` (`gmail.send` + `userinfo.email` + `openid`), `getGoogleCredentials`, `googleConfigurado`, `getRedirectUri` (default `PUBLIC_API_URL + /auth/google/callback`), `getFrontendUrl` (`FRONTEND_URL` → `APP_PUBLIC_URL` → `RP_ORIGIN` → `https://crmsumichen.com`), `crearOAuthClient` y `firmarState`/`verificarState` (JWT de 15 min que identifica al vendedor y evita CSRF).
- **`services/googleAuthServices.ts`**: `generarUrlAutorizacionService` (`access_type=offline` + `prompt='select_account consent'` para recibir `refresh_token` y permitir cambiar de cuenta), `procesarCallbackService` (intercambia `code`, obtiene el email con `userinfo.get` y guarda tokens), `estadoGoogleService`, `desconectarGoogleService` (revoca en Google + limpia columnas) y `getCuentaGmailVendedor`.
- **`services/gmailServices.ts`**: `enviarCorreoConGmail(...)` arma el MIME con `MailComposer` (firma inline por `cid` + adjuntos) y lo envía con `gmail.users.messages.send({ raw: base64url })`. Errores claros si el token dejó de ser válido.
- **`services/correosServices.ts`**: `enviarCorreoCliente` ahora lee el perfil con `getUsuarioConGoogleByIdDb` y, **si hay `google_refresh_token` + `google_email`, envía por Gmail**; si no, usa Resend (la validación de `RESEND_API_KEY` se movió al branch de Resend, ya que con Gmail no hace falta).
- **Módulo `/auth/google`** (`routes/googleAuthRoutes.ts` montado en `indexRoutes.ts`):
  - `GET /auth/google/url` (JWT) → devuelve `{ url }` de consentimiento (el frontend navega; una redirección completa no puede llevar el header `Authorization`).
  - `GET /auth/google/callback` (**público**) → Google vuelve aquí con `code`+`state`; guarda tokens y redirige a `FRONTEND_URL/#/configuracion?google=success|error`.
  - `GET /auth/google/status` (JWT) → `{ configurado, conectado, email }`.
  - `DELETE /auth/google` (JWT) → desconecta la cuenta.
- **Env nuevos** (`.env` y `.env.example`): `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URL`, `GOOGLE_STATE_SECRET` (opcional; si vacío usa `SUPABASE_JWT_SECRET`) y `FRONTEND_URL`.

#### Frontend
- **`hooks/useGoogleAuth.ts`** (nuevo): `useGoogleStatus` (query `["google-status"]`), `useConectarGoogle` (pide la URL y navega) y `useDesconectarGoogle` (invalida el estado).
- **`pages/configuracion/Configuracion.tsx`** (pestaña Perfil): tarjeta **"Cuenta de correo (Gmail)"** con botón "Conectar con Google", estado conectado (muestra el email + botón "Desconectar") y aviso si faltan credenciales en el backend. Al volver con `?google=success|error` muestra un toast y limpia el query (`useSearchParams`).
- **Tipos**: `UserData.google_email` (`context/types.ts`) + mapeo en `useUserData.ts`.

#### Cómo probar
1. Configuración → Perfil → "Conectar con Google" → iniciar sesión con la cuenta del vendedor y aceptar permisos.
2. Volver a la app (toast "¡Gmail conectado correctamente!"); verificar `SELECT google_email FROM vendedores WHERE ...`.
3. Detalle de cliente → "Enviar Email" → enviar → el correo debe salir desde la cuenta del vendedor y aparecer en su Gmail → "Enviados".
4. "Desconectar" y repetir → el envío vuelve a Resend.

#### ⚠ Notas / deuda
- **Configuración en Google Cloud** (una sola vez): crear el proyecto, habilitar **Gmail API**, configurar la pantalla de consentimiento y agregar los correos de los vendedores como **usuarios de prueba** (apps en modo Testing con cuentas `@gmail.com`). En "Credenciales" (Aplicación web) registrar el redirect EXACTO: dev `http://localhost:3000/auth/google/callback`, prod `https://crmsumichen.com/api/auth/google/callback`. Luego completar `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` en `backend/.env`.
- **Cambiar de correo**: el vendedor pulsa "Desconectar" y vuelve a "Conectar con Google"; `prompt='select_account'` fuerza el selector de cuentas de Google.
- El flujo no crea costo (API de Gmail gratuita, sujeta a cuotas). Requiere que cada vendedor tenga su cuenta de Google activa.
- Los tokens se guardan en texto plano en la DB (`select:false` solo evita exponerlos por API). Si en el futuro se quiere cifrado en reposo, añadir cifrado con una clave del `.env`.
- Para desplegar: recompilar y arrancar el backend (aplica la migración), configurar las 5 variables de Google en el `.env` del VPS y registrar el redirect de producción en Google Cloud.

### Punto 32 — Fix pantalla en blanco por Service Worker + ErrorBoundary (14/09) ✅ (lint + build OK frontend; `sw.js` verificado en `dist/`)

> **Resumen**: en producción la app se quedaba **en blanco** de forma intermitente. Causa raíz: el handler `fetch` del service worker (`public/sw.js`) devolvía `index.html` como fallback para **cualquier** recurso estático que fallara (incluidos `.js`/`.css`). Tras un deploy, el `index.html` cacheado referenciaba chunks con hash ya eliminados → la petición del `.js` fallaba → el SW respondía con HTML → el navegador intentaba ejecutar HTML como JavaScript → React nunca montaba. Como el SW quedaba instalado, el problema persistía hasta limpiar datos del sitio.

#### Cambios
- **`project/public/sw.js` reescrito**: se **eliminó por completo el handler `fetch`/de caché**. El CRM es *online-first* (los datos viven en la BD/API), así que el caché del app-shell no aportaba valor y era el 100% del problema. El SW ahora solo: `install` → `skipWaiting()`; `activate` → borra **todas** las cachés antiguas y `clients.claim()` (auto-repara clientes rotos); y los handlers `push`/`notificationclick`/`notificationclose` (sin cambios, Web Push intacto). Sin `respondWith`, es imposible responder con contenido equivocado.
- **Nuevo `project/src/components/ErrorBoundary.tsx`**: ErrorBoundary de React con botón **"Reparar y recargar"** que desregistra los service workers (`navigator.serviceWorker.getRegistrations()` → `unregister()`) y borra todas las cachés (`caches.keys()` → `delete()`) antes de recargar, además de "Solo recargar". Evita la pantalla en blanco sin mensaje.
- **`project/src/main.tsx`**: `<App />` envuelto en `<ErrorBoundary>`.

#### Deploy / operación
- Subir el `dist/` nuevo (incluye el `sw.js` corregido). Los clientes rotos se **auto-reparan**: al abrir, el navegador descarga el SW nuevo, que purga las cachés viejas; a la siguiente recarga la app vuelve a funcionar.
- En el VPS, servir `sw.js` con `Cache-Control: no-cache` para que las actualizaciones del SW lleguen rápido.

#### ⚠ Notas / deuda
- Se perdió el (poco útil) caché offline del app-shell: sin red la app ya no muestra el shell. Para el CRM (online-first) no aporta; si algún día se necesita modo offline real, migrar a `vite-plugin-pwa`/Workbox (que versiona el precache con hashes y hace el fallback de navegación correcto).
- `tsc --noEmit -p tsconfig.app.json` reporta errores **pre-existentes** en `src/utils/pedidos.ts` (no relacionados con este cambio); el script `build` del proyecto solo corre `vite build`, por lo que no bloquean el despliegue.

### Punto 33 — Fix de tipos en `utils/pedidos.ts` + `currentUser.rol` (14/09) ✅ (tsc de esos archivos limpio; lint + build OK)

> **Resumen**: `tsc --noEmit` reportaba 5 errores en `src/utils/pedidos.ts` (y 1 relacionado en `types/index.ts`). Se corrigieron sin cambios de runtime (solo tipos), bajando el total de errores de `tsc` de 48 a 19 (los 19 restantes son pre-existentes y ajenos: `Calendario.tsx`, `lib/biometric.ts`, `lib/push.ts`, `Analitica*`, recharts, etc.).

#### Cambios
- **`types/index.ts`**: `Pedido.transporte_detalle?: Transporte` → `Transporte | Partial<Transporte>`. Esto corrige el error TS2430 (`PedidoData` incorrectly extends `Pedido`, que redeclara el campo como `Partial<Transporte>`) y los dos TS2322 de `utils/pedidos.ts` (líneas 77 y 110) al pasar `pedidoData: Partial<Pedido>`.
- **`context/types.ts`**: `AuthContextType.currentUser` pasa de `User | null | undefined` a `(User & { rol?: "vendedor" | "admin"; supabase_id?: string }) | null | undefined`. En runtime `currentUser` es el perfil de `vendedores` (trae `rol`/`supabase_id`), no el `User` crudo de Supabase; el tipo ahora refleja eso y elimina los TS2551 de `currentUser.rol` en todo el frontend. **No se usó `user_metadata.rol`** a propósito (el rol autoritativo es el de la tabla, no el claim del JWT).
- **`utils/pedidos.ts`**: el parámetro `currentUser` de `abrirGmail` (tipo inline `User`) se amplió con `& { rol?: "vendedor" | "admin" }` (esa función es legacy/sin uso, pero el tipo estaba mal).
- **Modales con `currentUser = vendedor` (prop)**: se amplió el tipo de la prop `vendedor` con `& { rol?: "vendedor" | "admin" }` en `PedidosModal.tsx`, `PedidosDetailModal.tsx`, `ClienteDetalleModal.tsx` y `DashboarVendedorModal.tsx` (misma causa: la prop se tipaba como `User` sin `rol`).

#### Verificación
- `npx tsc --noEmit -p tsconfig.app.json`: **0 errores en `src/utils/pedidos.ts` y `src/types/index.ts`** y **0 errores de `Property 'rol'`** en todo el frontend.
- `npm run lint` 0 errores (solo 1 warning pre-existente en `ChatLista.tsx`) y `npm run build` OK.
- ⚠ Los 19 errores restantes de `tsc` son pre-existentes y ajenos a este arreglo; el `build` no los bloquea porque solo corre `vite build`.

### Punto 34 — Limpieza total de errores de `tsc` + warning de lint (14/09) ✅ (`tsc` 0 errores, `lint` 0 problemas, `build` OK)

> **Resumen**: se eliminaron los **19 errores pre-existentes** que quedaban tras el Punto 33, dejando `npx tsc --noEmit -p tsconfig.app.json` en **0 errores** y `npm run lint` en **0 problemas** (se quitó también el único warning). Solo cambios de tipos/robustez, sin cambios de comportamiento.

#### Fixes
- **`lib/biometric.ts` y `lib/push.ts`** (7 errores): `Uint8Array` se infiere como `Uint8Array<ArrayBufferLike>`, que ya no es asignable a `BufferSource` (TS 5.7+ exige `ArrayBuffer`). Se tipó el retorno de `base64UrlToUint8Array` / `urlBase64ToUint8Array` como `Uint8Array<ArrayBuffer>` y se casteó `pubKeyCredParams as PublicKeyCredentialParameters[]`.
- **`pages/analitica/Analitica.tsx` y `AnaliticaModal.tsx`** (2): `m.clientes` es `number | undefined` → `(m.clientes ?? 0) > 0`.
- **`pages/dashboard/DashboardVendedor.tsx`** (1): el `formatter` del `<Tooltip>` de recharts se tipó `(value)` (sin `: number`) y se usa `Number(value).toLocaleString(...)`, que satisface el tipo `Formatter` de recharts.
- **`components/forms/ActividadDetalleModal.tsx`** (1): se quitó la comparación muerta `estado === "resuelto"` (el estado solo es `programada|completada|cancelada|pendiente`; "resuelto" es de tickets).
- **`pages/reuniones/Calendario.tsx`** (8): el componente DnD ahora se instancia con el genérico del evento: `withDragAndDrop<CalendarioEvento>(Calendar)`, lo que alinea `events`, `components`, `onSelectEvent`, `onEventDrop` y `onEventResize` (antes el evento se tipaba como `object`). El `resource` se castea a `ReunionCalendario` (el backend sí devuelve `fecha_creacion`/`fecha_actualizacion`).
- **`pages/chat/ChatLista.tsx`** (1 warning): `conversacionesFiltradas` se envolvió en `React.useMemo(..., [conversaciones, search])` para estabilizar la dependencia de `gruposPorVendedor`.
